					...
					children={(field) => {
						...
						return (
							<Field data-invalid={isInvalid} className="flex-1">
								...
								<form.Subscribe
									selector={(state) => state.values.section.endDate}
									children={(endDate) => (
										<Popover>
											<PopoverTrigger
												render={
											...
											>
												<CalendarIcon />
												<span>
													{`${format(field.state.value, "LLL dd, y")} - ${format(endDate, "LLL dd, y")}`}
												</span>
											</PopoverTrigger>
											<PopoverContent className="w-auto h-78" align="start">
												<Calendar
													...
													selected={{ from: field.state.value, to: endDate }}
													onSelect={(range) => {
														if (range?.from) field.handleChange(range.from);
														if (range?.to)
															form.setFieldValue("section.endDate", range.to);
													}}
													numberOfMonths={2}
												/>
											</PopoverContent>
							...
							<Field data-invalid={isInvalid} orientation="horizontal">
								<FieldContent>
									...
									<FieldLabel htmlFor={field.name}>
										<FieldDescription>
											Toggle this setting on if the course is a class that does
											not take place in a location.
										</FieldDescription>
									</FieldLabel>
									{isInvalid && <FieldError errors={field.state.meta.errors} />}
								</FieldContent>
													...
													<div className="flex flex-col gap-2">
														<div className="flex flex-col">
															...
															<span className="text-muted-foreground text-sm">
																Add the instructor names who teach this section.
															</span>
														</div>
														{form
															.getFieldValue(
